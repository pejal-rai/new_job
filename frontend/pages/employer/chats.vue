<!-- pages/employer/chats.vue -->
<template>
  <div class="container mx-auto p-6">

    <h1 class="text-3xl font-bold text-gray-900 mb-6">Employer Chat Dashboard</h1>
      ...
      <div class="bg-white/75 shadow-lg rounded-lg p-6 w-1/3">
        ...
        <div v-else class="space-y-2">
          ...
          >
            ...
            <p class="text-gray-600 text-sm">User: {{ chat.user_name }}</p>
...
const fetchChats = async () => {
  try {
    const response = await $fetch(`${config.public.apiBase}/chat/employer`, {
      method: "GET",
      credentials: "include",
    });
    if (response.success) {
...
onMounted(async () => {
  checkFontAwesome();
  if (userStore.user?.role !== "employer") {
